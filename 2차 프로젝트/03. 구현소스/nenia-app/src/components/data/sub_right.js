// nenia 서브가로스크롤 데이터

export const subRightData = {
  way: [
    { 
      tit: "NON-GMO 원료만을 엄선하여 사용합니다",
      img: "/images/way/right_scroll_img/img1.png",
      desc: "수입산 옥수수전분, 물엿, 변성전분, 대두단백,액상과당 등 GMO우려가 있는 원재료는 사용하지 않습니다. 복합 원재료의 경우도 원료 자체에 GMO의 위험이 있는 것은 배제합니다.",
    
    },
    {
      tit: "무농약, 유기농 인증 우리밀을 사용합니다.",
      img: "/images/way/right_scroll_img/img2.png",
      desc: "일부제품(우리밀 통밀가루, 백밀가루) 외 모든 제품에 무농약, 유기농 인증 우리밀을 사용하고 있습니다.",
     
    },
    {
      tit: "가능한 모든 유해한 화학적 합성첨가물을 넣지 않고자 합니다.",
      img: "/images/way/right_scroll_img/img3.png",
      desc: "그 동안 많은 식품업체들이 원료로 원가를 절감하고 생산 효율을 높이는 유통의 편리성을 도모해왔습니다. 네니아는 우리땅에서 나는 우리밀과 우리 농산물을 원료로, 유해한 화학적 식품첨가물을 넣지 않고 자연적인 먹을거리에 가까운 식품을 만들고자 노력합니다.",
     
    },
    {
      tit: "100% 전성분 공개를 원칙으로 합니다.",
      img: "/images/way/right_scroll_img/img4.png",
      desc: "소량으로 들어가 있는 원재료까지도 ‘~외’로 감추지 않고 100% 공개합니다.",
     
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
     
    },
    {
      tit: "우리 땅에서 나는 우리 농산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img6.png",
      desc: "생산이 불가능한 원료(설탕, 후추, 초콜렛, 허브류 등)를 제외하고 국내산으로 대체 가능한 모든 원료는 가까운 먹을거리인 우리 농산물을 사용합니다.",
     
    },
    {
      tit: "최선을 찾아 직접 수입합니다.",
      img: "/images/way/right_scroll_img/img7.png",
      desc: "국내 수급이 불안정한 원료는 국외로 방문하여 최고의 품질을 확인하고 직접 수입 합니다.",
     
    },
  ],
  wheat: [
    {
      tit: "100% 국내산",
      img: "/images/wheat/right_scroll_img/img1.png",
      desc: "우리밀은 우리의 들녘에서 우리의 공기와 물을 먹으며 자란 우리 곡물입니다.",
    },
    {
      tit: "농가소득 기여",
      img: "/images/wheat/right_scroll_img/img2.png",
      desc: "겨울철 유휴농지를 활용하여 우리밀 농가소득 증대에 기여 합니다.",
    },
    {
      tit: "생태환경 보존",
      img: "/images/wheat/right_scroll_img/img3.png",
      desc: "우리밀 8만ha 재배 시 산소 23만 톤 방출, 1ha당 100kg의 미세먼지를 제거합니다.",
    },
    {
      tit: "식량안보 파수꾼",
      img: "/images/wheat/right_scroll_img/img4.png",
      desc: "대한민국 제2의 식량으로 우리밀 소비 확대는 식량자급률 향상에 기여 합니다.",
    },
    {
      tit: "환경 보호",
      img: "/images/wheat/right_scroll_img/img5.png",
      desc: "수입밀(미국, 캐나다 등) 대비 이동거리가 짧아 탄소 저감효과에 기여 합니다.",
    },
    {
      tit: "품질 제고 노력",
      img: "/images/wheat/right_scroll_img/img6.png",
      desc: "철저한 품질관리로 안정성 및 가공 품질 적성을 충족한 밀입니다.",
    },
  
   
    
  ],
  ecomeal: [
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
    {
      tit: "가능한 친환경 농산물과 무항생제 축산물을 사용합니다.",
      img: "/images/way/right_scroll_img/img5.png",
      desc: "연중수급이 불가능하여 사용이 어려운 경우를 제외하고 최대한 친환경 농산물과 무항생제 축산물을 사용합니다.",
    },
   
  ],
};


